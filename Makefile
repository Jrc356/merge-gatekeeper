BINARY_NAME=merge-gatekeeper

.PHONY: build
build: 
	GO111MODULE=on LANG=en_US.UTF-8 CGO_ENABLED=0 go build ./cmd/${BINARY_NAME}
...
.PHONY: run
run: build
	./${BINARY_NAME} validate --ref ${REF} --token ${GITHUB_TOKEN} -r ${REPO} --ignored "$${IGNORED_JOBS}"